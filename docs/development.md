...
`make up` also builds and starts `bot/BOT_MAI`. In the development Compose environment, the bot account, tokens, `MENTION_MESSAGE_CREATED` subscription, and activation are configured automatically; an external tunnel is not required. See [`bot/BOT_MAI/README.md`](../bot/BOT_MAI/README.md) for details.
