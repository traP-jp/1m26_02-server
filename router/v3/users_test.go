...
func TestHandlers_CreateUser(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		require.NoError(t, err)
		subscriptions, err := env.Repository.GetChannelSubscriptions(context.Background(), repository.ChannelSubscriptionQuery{}.SetUser(createdUser.GetID()))
		require.NoError(t, err)
		require.Len(t, subscriptions, 1)
		assert.Equal(t, general.ID, subscriptions[0].ChannelID)
		assert.Equal(t, model.ChannelSubscribeLevelMarkAndNotify, subscriptions[0].GetLevel())
